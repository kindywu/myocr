...
data class DrugInfo(
    ...
    /** 是否所有必填字段都已填写 */
    val isComplete: Boolean get() = drugName.isNotBlank()

    /** 是否有任何字段有值 */
    val hasAnyValue: Boolean get() = drugName.isNotBlank() || expiryDate.isNotBlank()
            || manufacturer.isNotBlank() || batchNumber.isNotBlank()

    /** 合并两组药品信息（新值覆盖旧值） */
    fun mergeWith(latest: DrugInfo): DrugInfo = copy(
